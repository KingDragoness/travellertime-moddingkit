...
namespace Scripts.Utility
{
    public class AutoLODGrouper : MonoBehaviour
    {
        ...
        public void AutoLOD()
        {
            ...
            all_LOD0 = GetComponentsInChildren<MeshRenderer>().Where(x => x.name.Contains("LOD0")).ToList();
            all_LOD1 = GetComponentsInChildren<MeshRenderer>().Where(x => x.name.Contains("LOD1")).ToList();
            all_LOD2 = GetComponentsInChildren<MeshRenderer>().Where(x => x.name.Contains("LOD2")).ToList();
            ...
            foreach (LODGroup lodgroup in allLODGroups_Created)
            {
                ...
                {
                    if (lodinfo.rendr.transform.parent != this.transform)
                        lodinfo.rendr.transform.parent.SetParent(lodgroup.transform);
